  ...
  render () {
    ...
    return (
      <div className='main__container'>
        <h1
          className='container__heading truncate--lg'
          title={jsonRssData ? jsonRssData.feed.url : "Hello. I'm Arthur!"}
        >
          {jsonRssData ? jsonRssData.feed.url : "Hello. I'm Arthur!"}
        </h1>
        <div className='rss__container'>
          {jsonRssData ? (
            jsonRssData.items.map((item, index) => (
              <div className='container__cards' key={index.toString()}>
                <p className='container__cards--title'>{item.title}</p>
